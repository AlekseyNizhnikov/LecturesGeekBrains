...
**Git branch name_branch** - команда терминала, которая позволяет создать еще одну ветку состояния репозитория, например для выгрузки заказчику проекта по-частям, либо для параллельной работы над одним проектом в команде.

**Git checkout name_branch** - команда треминала_ .которая позволяет перейти к нужной ветки в дереве коммитов, либо к конкретному коммиту, указав его код или первые 5 - 6 символов кода.
